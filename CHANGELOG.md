...
- Re-attach network capture observers across page swaps so requests
  fired after `Navigate`, `OpenTab`, or `SwitchTab` are captured.
- Adopt the shared golangci-lint configuration and resolve the
  reported gocritic and gosec findings.

### Changed

- Require signed Git operations for Relicta-managed releases.
- Move code-level security ownership to nox and update CI security
  workflows.
- Adopt the shared Go CI workflow.
